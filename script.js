...
class Card {
  ...
  getOwnObj() {return MYLOCALSTORAGE.ALLCARDSOBJS.find((element) => {return element.id == this.id;})}
  ...
  initialSettings(obj) {
    ...
    MYLOCALSTORAGE.ALLCARDSOBJS.push(obj);
    this.markSorteds();
    if (this.allOk) MYLOCALSTORAGE.saveInLocalStorage();
  ...
  update(name, arrOfNumbers, changeId = false) {
    ...
    this.markSorteds()
    MYLOCALSTORAGE.saveInLocalStorage()
    allCards.showHideAddFirstCard()
  }
  ...
  markSorteds(sorteds) {
    ...
    for (let i = MYLOCALSTORAGE.SORTEDBALLS.length - 1; i >= 0; i--) {
      const index = this.numbers.indexOf(MYLOCALSTORAGE.SORTEDBALLS[i])
      if (index > -1) {
        count++;
...
const cardInput = {
  ...
  modified : {
    ...
    set2false() {
      cardInput.modified.name = false;
      cardInput.modified.items = false;
    }
  },
  ...
  btnRandom : document.getElementById("cardInputRandomBtn"),
  numbers : [],
  ...
  initialSettings() {
    const checkInput = (input) => {
      // if (input.validity.valid) {
        if (cardInput.validity()) {
          cardInput.enableSave();
          return;
        } 
      // }
      cardInput.disableSave()
    }
    ...
    // setTimeout(function() { // ! todelete,  just to test
    //   cardInput.open(MYLOCALSTORAGE.ALLCARDSOBJS[1])
    // }, 500)

    cardInput.btnRandom.addEventListener("click", function() {
      cardInput.randomValues();
    })


    this.btnRandom.addEventListener('mouseenter', function() {
      cardInput.inputs.forEach(element => element.classList.add("shaking"))
    })
    this.btnRandom.addEventListener('mouseleave', function() {
      cardInput.inputs.forEach(element => element.classList.remove("shaking"))
    })
  },
  open(obj) {
    cardInput.currentObj = obj;
    cardInput.inputs.forEach(element => { element.classList.remove('equal'); })
    cardInput.numbers = obj.numbers.concat();
    cardInput.modified.set2false();

    if (obj.allOk) {
      cardInput.btnCancel.onclick = function () {
        if (!(cardInput.modified.some())) {
          cardInput.close();
          return;
        }
        myConfirm.open(
    ...
    if (cardInput.validity() && cardInput.modified.some()) {
      cardInput.enableSave()
    } else {
      cardInput.disableSave();
    }
  ...
  save(obj) {
    ...
    if (obj.allOk) {
      myConfirm.open(
        ...
        "This action will change the ID of the current card",
        myConfirm.btnCancel,
        function() {
  ...
  randomValues() {
    cardInput.numbers = []
    cardInput.inputs.forEach(element => { element.classList.remove('equal'); })
    const inputs = cardInput.inputs;
    const getRandom = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
    const execute = function () {
      inputs.forEach(element => {
        let value = getRandom(parseInt(element.min), parseInt(element.max))
        while(true) {
          value = getRandom(parseInt(element.min), parseInt(element.max))
          if (!(cardInput.numbers.includes(value))) break
        }
        cardInput.numbers.push(value)
  
        element.value = value
      })
    }

    // ask first if cardImput has values
    if (cardInput.currentObj.allOk || cardInput.modified.items) {
      myConfirm.open(
        "Attention!",
        "All the values will be changed, but not saved",
        myConfirm.btnCancel,
        function() {execute()},
        function() {}
      )
...
onload = function() {
  MYLOCALSTORAGE.initialSettings();
  MYLOCALSTORAGE.findContent();
...
const MYLOCALSTORAGE =  {
  ...
  initialSettings() {
    this.content.ALLCARDSOBJS = this.ALLCARDSOBJS;
    this.content.SORTEDBALLS = this.SORTEDBALLS;
    this.content.ALLAPPEARENCEVALUES = this.ALLAPPEARENCEVALUES;
  },
  findContent() {
    ...
    const foundContent = localStorage.getItem("contentBingoOfLocalStorage")

    if (!(foundContent)) { return }
    
    const foundContentObj = JSON.parse(foundContent)

    if (foundContentObj.ALLCARDSOBJS) {
      foundContentObj.ALLCARDSOBJS.forEach((element, i) => {
        new Card(element.id, element.name, element.numbers)
      })
    }

    if (foundContentObj.SORTEDBALLS) {
      foundContentObj.SORTEDBALLS.map((element) => {
        pushBall(element);
        return element
      })
    }

    if (foundContentObj.ALLAPPEARENCEVALUES) {
      for ([key, value] of Object.entries(foundContentObj.ALLAPPEARENCEVALUES)) {
        appearence[key].selected = value
  ...
  saveInLocalStorage() {
    ...
    localStorage.setItem("contentBingoOfLocalStorage", JSON.stringify(MYLOCALSTORAGE.content))
  },
  resetjson(cards = true, balls = true, appearence = true) {
    if (cards) {
      MYLOCALSTORAGE.ALLCARDSOBJS = JSON.parse('[{"id":"181a8a3781f","name":"MARIA","numbers":[14,10,7,11,15,20,27,23,28,25,32,42,34,33,52,55,58,56,53,71,64,69,72,66],"allOk":true},{"id":"1gek9c3j5","name":"CARLOS","numbers":[15,8,9,10,11,27,24,28,20,30,40,37,44,36,54,56,49,53,55,67,65,63,69,75],"allOk":true}]')
    }
    if (balls) {
      MYLOCALSTORAGE.SORTEDBALLS = JSON.parse('[1,44,33,22,16,34,22,3,6,14,71,32,23]');
    }
    if (appearence) {
      MYLOCALSTORAGE.ALLAPPEARENCEVALUES = {
    ...
    MYLOCALSTORAGE.saveInLocalStorage()
    // console.log(MYLOCALSTORAGE.content)
    location.reload()
  }
}

const allCards = {
  firstCardBtn : document.getElementById("firstCard"),
  initialSettings() {
    //firstCard
    allCards.firstCardBtn.addEventListener("click", allCards.addFirstCard)
    // myConfirm.btnOk.addEventListener("click", allCards.showHideAddFirstCard) // added to myconfirm

  },
  addFirstCard() {
    addCard();
  },
  showHideAddFirstCard() {
    setTimeout(function() {
      if(MYLOCALSTORAGE.ALLCARDSOBJS.length >= 1) {
        allCards.firstCardBtn.classList.remove("visible")
      } else {
        allCards.firstCardBtn.classList.add("visible")
      }
      // console.log('1') // ? can be executed several times
    }, 100)
  }
}
allCards.initialSettings()
...
function removeFromALLCARDSOBJS(id) {
  const index = MYLOCALSTORAGE.ALLCARDSOBJS.findIndex((element) => {
    return element.id == id
  })

  MYLOCALSTORAGE.ALLCARDSOBJS.splice(index, 1)
  MYLOCALSTORAGE.saveInLocalStorage()
}
...
function createSomeCards() {
  ...
  for (let i = 0; i < length; i++){
    MYLOCALSTORAGE.ALLCARDSOBJS.push(cards[i])
  }
}
...
function pushBall(number = inputBall.value) {
  ...
  MYLOCALSTORAGE.SORTEDBALLS.push(Number(number))
  MYLOCALSTORAGE.saveInLocalStorage()
  ...
  newBallHistoric.querySelector('.tagOrder').innerHTML = MYLOCALSTORAGE.SORTEDBALLS.length + '°'

  allBallsHistoric.insertBefore(newBallHistoric, allBallsHistoric.firstChild)

  inputBall.value = '';
  tagLetterInput.innerHTML = ''
  tagOrderInput.innerHTML = MYLOCALSTORAGE.SORTEDBALLS.length + 1 + '°';
  controlThingsOfInputBall()

  MYLOCALSTORAGE.ALLCARDSOBJS.forEach((element) => {
    element.markSorteds(MYLOCALSTORAGE.SORTEDBALLS)
  })
...
function undoBall() {
  ...
  toUndo = setTimeout(() => {
    ...
    number = MYLOCALSTORAGE.SORTEDBALLS.pop()
    MYLOCALSTORAGE.saveInLocalStorage();
    markSortedBalls()
    tagOrderInput.innerHTML = MYLOCALSTORAGE.SORTEDBALLS.length + 1 + 'º';
    if (MYLOCALSTORAGE.SORTEDBALLS.length) {undoBallBtn.classList.remove('blocked')}
...
function markSortedBalls() {
  balls.forEach(element => {
    if (MYLOCALSTORAGE.SORTEDBALLS.includes(Number(element.innerHTML))) {
...
function controlThingsOfInputBall() {
  ...
  const order = MYLOCALSTORAGE.SORTEDBALLS.length + 1 + '°'
  ...
  if (MYLOCALSTORAGE.SORTEDBALLS.includes(number) || number <= 0 || number > 75) {
...
const pageSettingsSection = {
  ...
  settings : [], //maybe 2 delete
  resetBtn : document.getElementById("sttResetBtn"),
  initialSettings() {
  ...
  reset() {
    myConfirm.open(
      ...
      "<br><br>This action will delete all data including cards, drawn numbers and personalization settings." +
      "<br><br>* This action cannot be undone." + 
      "<br>* The page will be reloaded",
      myConfirm.btnCancel,
      function() {
        if (localStorage != undefined) {
          localStorage.setItem("contentBingoOfLocalStorage", "")
...
const appearence = {
  primaryColor : {
    ...
    setValue() {
      ...
      MYLOCALSTORAGE.ALLAPPEARENCEVALUES.primaryColor = value
      MYLOCALSTORAGE.saveInLocalStorage()
    }
  },
  theme : {
    ...
    setValue() {
      ...
      MYLOCALSTORAGE.ALLAPPEARENCEVALUES.theme = value
      MYLOCALSTORAGE.saveInLocalStorage()
    }
  },
  sortedColors : {
    ...
    setValue() {
      ...
      MYLOCALSTORAGE.ALLAPPEARENCEVALUES.sortedColors = values
      MYLOCALSTORAGE.saveInLocalStorage();
    }
  },
  initialSettings() {
...
const sttExport = {
  ...
  export() {
    ...
    if (sttExport.checkboxes[0].getAttribute('ischecked') == 'true') { exportContet.balls = MYLOCALSTORAGE.SORTEDBALLS; }
    if (sttExport.checkboxes[1].getAttribute('ischecked') == 'true') { exportContet.cards = MYLOCALSTORAGE.ALLCARDSOBJS}
...
const sttImport = {
  ...
  pushFoundBall(ballNumber) {
    ...
    if (!(MYLOCALSTORAGE.SORTEDBALLS.includes(ballNumber))) {
  ...
  pushFoundCard(card) {
    ...
    let includes = MYLOCALSTORAGE.ALLCARDSOBJS.some(element => {
  ...
  addBalls(notify = true) {
    let toAdd = sttImport.foundPreview.balls.filter(element => {
      return (!(MYLOCALSTORAGE.SORTEDBALLS.includes(element)));
    })
    toAdd = new Set(toAdd)
  ...
  addCards(notify = true) {
    let toAdd = sttImport.foundPreview.cards.filter(element => {
      for (let i = MYLOCALSTORAGE.ALLCARDSOBJS.length - 1; i >= 0; i--) {
        if (element.id == MYLOCALSTORAGE.ALLCARDSOBJS[i].id) return false
      }
      return true
    })
  ...
  replaceCards() {
    for (let i = MYLOCALSTORAGE.ALLCARDSOBJS.length - 1; i >= 0; i--) {
      MYLOCALSTORAGE.ALLCARDSOBJS[i].remove(false)
    }
